...
def polrange(Q,Q1,hx,hy,m):
   ...
   Authors: Thomas R. Cameron,  Department of Mathematics and Computer Science, Davidson College
            Based on the original work (see num_range.py) of Fatouros and Panyiotis Psarrakos,
...
#   Companion matrices
    ...
    for j in range(1,m-1):
        C0=np.concatenate((C0,-Q1[j]),axis=1)

    [l1,l2]=C0.shape
    C1=np.concatenate((np.concatenate((np.zeros((l2-l1,l1),dtype=complex),np.eye(l2-l1,dtype=complex)),axis=1),C0),axis=0)

#   Eigenvalues
    temp=np.linalg.eigvals(C)
    real_eig=np.real(temp)
    imag_eig=np.imag(temp)
    temp=np.linalg.eigvals(C1)
    real_eig1=np.real(temp)
    imag_eig1=np.imag(temp)
...
#   Constructing the grid
    temp=max(max(abs(real_eig)),max(abs(real_eig1)))
    temp=np.ceil(temp)
    xrang=[-temp,temp]
    temp=max(max(abs(imag_eig)),max(abs(imag_eig1)))
    temp=np.ceil(temp)
    yrang=[-temp,temp]
...
#   Computing numerical range of Q
...
#   Plotting figure
    plt.figure()
    plt.contour(X,Y,Z.T,[0],linewidths=2,colors='b')
    ...
    plt.plot(real_eig,imag_eig,'bo',markersize=2)
...
#   Computing numerical range of Q1
    for x in np.arange(xrang[0],xrang[1]+hx,hx):
        I=int(round((x-xrang[0])/hx,0))
...
#   Plotting figure
    plt.contour(X,Y,Z.T,[0],linewidths=2,colors='r')
    plt.plot(real_eig1,imag_eig1,'ro',markersize=2)
    plt.show()
...
#   self-adjoint monic matrix polynomial and
#   its derivative
###############################################
def main():
#   grab arguments
    ...
    for k in range(1,m+1):
        Q1.append((k*1.0/m)*Q[k])

    hx=0.01
    hy=0.01
    polrange(Q,Q1,hx,hy,m)
